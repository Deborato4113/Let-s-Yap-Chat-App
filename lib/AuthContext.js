...
export function AuthProvider({ children }) {
  ...
  // Used after a successful Firebase sign-in (Google or Firebase email/
  // password) - our /api/auth/firebase route already verified the Firebase ID
  // token server-side and exchanged it for our own app JWT, so from here on
  // this is identical to a regular login: same token shape, same localStorage
  // keys, same socket auth downstream.
  const loginWithFirebase = useCallback((data) => {
    localStorage.setItem("yap_token", data.token);
    try {
      localStorage.setItem("yap_user", JSON.stringify(stripForStorage(data.user)));
    } catch {
  ...
  return (
    <AuthContext.Provider value={{ user, token, loading, login, register, loginWithFirebase, logout, updateUser }}>
      {children}
